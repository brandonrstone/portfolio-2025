...
export function ThemeToggle({ className, button = false }: ThemeToggleProps) {
  ...
  return button ? (
    <button
      className={clsx('relative flex items-center w-14 h-8 rounded-full px-1 cursor-pointer transition-colors duration-700 ease-in-out focus:outline-none', isDark ? 'bg-gray-800' : 'bg-green-300', className)} onClick={toggleTheme} aria-label='Toggle theme'>
    ...
    <button className={clsx('rounded-full cursor-pointer transition-colors duration-700 ease-in-out', isDark ? 'hover:text-yellow-100' : 'hover:text-blue-800/70', className)} onClick={toggleTheme} aria-label='Toggle theme'>
      {isDark ? (
